	...
	<body>
		...
		<div id="list-board" class="content scaffold-list" role="main">
			...
			<table>
				<thead>
					<tr>
                        <g:sortableColumn property="name" title="${message(code: 'board.name.label', default: 'Name')}" />
						<g:sortableColumn property="description" title="${message(code: 'board.description.label', default: 'Description')}" />
				...
				<g:each in="${boardInstanceList}" status="i" var="boardInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
                        <td><g:link action="show" id="${boardInstance.id}">${fieldValue(bean: boardInstance, field: "name")}</g:link></td>
			...
			<div class="pagination">
				<g:paginate total="${boardInstanceTotal}" />
			</div>
		</div>
	</body>
</html>
